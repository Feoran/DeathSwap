...
execute if entity @e[tag=game_core,scores={ds_crazy=1}] run execute as @a run loot give @s[tag=ingame] loot deathswap:crazyitem/crazy
execute if entity @e[tag=game_core,scores={ds_crazy=1}] run execute as @a run loot give @s[tag=ingame] loot deathswap:crazyitem/crazy
execute if entity @e[tag=game_core,scores={ds_crazy=1}] run tellraw @a[tag=ingame] [{"text":"[!]疯狂模式下，你的对手可能获取比你更好的强力装备！","color":"red","bold":true,"italic":false}]
execute if entity @e[tag=game_core,scores={ds_night=1}] run tellraw @a[tag=ingame] [{"text":"[!]永夜模式下，怪物将会不受限制的生成！","color":"red","bold":true,"italic":false}]